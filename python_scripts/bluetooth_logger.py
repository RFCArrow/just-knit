...
def main():
    ...
    try:
        ...
        directory = "/home/rob/just-knit/csv/"
        filename = directory+timestr+"_datalog"
        ...
        def received(data):
            dataString = ('{0}'.format(data))
            formattedData = dataString.replace(';', '\r\n')
            with open(filename, "a") as datalog:
                datalog.write(formattedData)
        # Turn on notification of RX characteristics using the callback above.
        print('Subscribing to RX characteristic changes...')
        rx.start_notify(received)

        # Now just wait for 30 seconds to receive data.
        gobble = raw_input("Press any key to stop recording...") 
        # Stop logging
        tx.write_value('e\r\n')
